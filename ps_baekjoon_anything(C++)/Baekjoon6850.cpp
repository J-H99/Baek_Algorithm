...
int main() {
    int N, next = 2, dest_point, starting_point, xPoint1, xPoint2, xPoint3, yPoint1, yPoint2, yPoint3, hull_number;
    cin >> N;
    ...
    xPoint1 = point_arr[0].x;
    yPoint1 = point_arr[0].y;

    xPoint2 = point_arr[convex_hull.top()].x;
    yPoint2 = point_arr[convex_hull.top()].y;

    convex_hull.pop();

    xPoint3 = point_arr[convex_hull.top()].x;
    yPoint3 = point_arr[convex_hull.top()].y;

    convex_hull.pop();
    sum = abs((xPoint2 - xPoint1) * (yPoint3 - yPoint1) - (xPoint3 - xPoint1) * (yPoint2 - yPoint1)) / 2;
    while (convex_hull.size() >= 2) {
        xPoint2 = xPoint3;
        yPoint2 = yPoint3;
